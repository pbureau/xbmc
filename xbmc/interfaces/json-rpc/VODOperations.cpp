...
JSONRPC_STATUS CVODOperations::ActionSearch(const std::string &method, ITransportLayer *transport, IClient *client, const CVariant &parameterObject, CVariant &result)
{
#if 0
  CLog::Log(LOGDEBUG,"%s::%s method: %s", __FILE__, __FUNCTION__, method.c_str());
  //CLog::Log(LOGDEBUG,"%s::%s param docid: %s", __FILE__, __FUNCTION__, parameterObject["docid"].asString().c_str());
  ...
  /* If we have a proper results array, send it to the VOD window */
  if (parameterObject.isMember("results") && parameterObject["results"].isArray())
  {
      CGUIMessage msg(GUI_MSG_VOD_ACTION, 0, 0);
      msg.SetPointer((void*)&parameterObject);
      CApplicationMessenger::GetInstance().SendGUIMessage(msg, WINDOW_MUSIC_NAV, true);
      result["moncul"] = "toto";
  }
  else
      result["moncul"] = "fail epic";
  return OK;
}

JSONRPC_STATUS CVODOperations::GetProfiles(const std::string &method, ITransportLayer *transport, IClient *client, const CVariant &parameterObject, CVariant &result)
{
...
JSONRPC_STATUS CVODOperations::GetCurrentProfile(const std::string &method, ITransportLayer *transport, IClient *client, const CVariant &parameterObject, CVariant &result)
{
...
JSONRPC_STATUS CVODOperations::LoadProfile(const std::string &method, ITransportLayer *transport, IClient *client, const CVariant &parameterObject, CVariant &result)
{
